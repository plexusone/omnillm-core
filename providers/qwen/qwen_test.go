// Package qwen provides unit tests for the Qwen API client using a mock HTTP server.
package qwen
...
// chatCompletionResponse builds a valid Qwen completion JSON response.
...
func TestQwenClient_New_DefaultBaseURL(t *testing.T) {
	c := New("test-key", "", nil)
...
func TestQwenClient_New_CustomBaseURL(t *testing.T) {
...
func TestQwenClient_Name(t *testing.T) {
	c := New("key", "", nil)
	if c.Name() != "qwen" {
		t.Errorf("Name() = %q, want qwen", c.Name())
	}
}

func TestQwenClient_CreateCompletion_ValidationErrors(t *testing.T) {
	c := New("key", "", nil)
	...
	t.Run("empty messages", func(t *testing.T) {
		_, err := c.CreateCompletion(ctx, &Request{Model: "qwen-plus"})
...
func TestQwenClient_CreateCompletion_Success(t *testing.T) {
	srv := newTestServer(t, http.StatusOK, chatCompletionResponse("qwen-plus", "test successful"))
	defer srv.Close()
...
func TestQwenClient_CreateCompletion_APIError(t *testing.T) {
...
func TestQwenClient_CreateCompletion_AuthorizationHeader(t *testing.T) {
	var capturedHeader string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		capturedHeader = r.Header.Get("Authorization")
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(chatCompletionResponse("qwen-plus", "ok")))
	}))
	defer srv.Close()
...
func TestQwenClient_CreateCompletion_StreamSetToFalse(t *testing.T) {
...
func TestQwenClient_CreateCompletionStream_ValidationErrors(t *testing.T) {
	c := New("key", "", nil)
	ctx := context.Background()
	...
	t.Run("empty messages", func(t *testing.T) {
		_, err := c.CreateCompletionStream(ctx, &Request{Model: "qwen-plus"})
		if err == nil || !strings.Contains(err.Error(), "messages cannot be empty") {
			t.Errorf("expected 'messages cannot be empty' error, got %v", err)
		}
	})
...
func TestQwenClient_CreateCompletionStream_Success(t *testing.T) {
	srv := newStreamServer(t, "qwen-plus")
	defer srv.Close()
...
func TestQwenStream_Close(t *testing.T) {
	srv := newStreamServer(t, "qwen-plus")
	defer srv.Close()
...
func TestQwenStream_RecvAfterClose(t *testing.T) {
	srv := newStreamServer(t, "qwen-plus")
	defer srv.Close()
...
func TestQwenProvider_CreateChatCompletion_Success(t *testing.T) {
	srv := newTestServer(t, http.StatusOK, chatCompletionResponse("qwen-plus", "hello there"))
	defer srv.Close()
...
func TestQwenProvider_CreateChatCompletionStream_Success(t *testing.T) {
	srv := newStreamServer(t, "qwen-plus")
	defer srv.Close()
...
func TestQwenProvider_CreateChatCompletion_EmptyChoices(t *testing.T) {
	body := `{"id":"x","object":"chat.completion","created":1,"model":"qwen-plus","choices":[],"usage":{}}`
...
func TestQwenClient_CustomHTTPClient(t *testing.T) {
	srv := newTestServer(t, http.StatusOK, chatCompletionResponse("qwen-plus", "ok"))
	defer srv.Close()
